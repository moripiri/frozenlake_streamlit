...
st.set_page_config(page_title="dyna-q", page_icon="🎲")

st.title("6. Dyna-Q")

st.markdown(
    '''
    This demo shows Dyna-Q in FrozenLake-v1. 
...
model_n = st.sidebar.number_input(
        "model_n",
        value=10,
        min_value=1,
        ...
        step=1,
        key="model_n",
        help="number of q-learning from model-generated sample"
    )
...
if reset:
    ...
    model_r = np.zeros([state_dim, action_dim])
    model_ns = np.zeros([state_dim, action_dim])
    ...
    with st.empty():
        for i in range(max_episode):
            ...
            for _ in range(model_n):
                ...
                sample_s = states[sample]
                sample_a = actions[sample]

                sample_r = (model_r[sample_s, sample_a])
                sample_ns = int(model_ns[sample_s, sample_a])
                q[sample_s, sample_a] = q[sample_s, sample_a] + alpha * (
                            sample_r + gamma * np.max(q[sample_ns, :]) - q[sample_s, sample_a])
